...
import SwiperLib from "../content/libraries/thejsguy-swiper.mdx";
    ...
    <Routes>
      {links.map((link) => {
        if (!link.children) {
          ...
          return (
            <Route key={link.key} path={link.key} element={<Component />} />
          );
        }
